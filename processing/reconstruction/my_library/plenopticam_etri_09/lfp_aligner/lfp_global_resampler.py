# local imports
from plenopticam_etri_09.lfp_aligner.lfp_microlenses import LfpMicroLenses
from plenopticam_etri_09.lfp_calibrator.grid_fitter import GridFitter
...
class LfpGlobalResampler(LfpMicroLenses):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # print("mslee21/LfpGlobalResampler/self._lfp_img.copy()",self._lfp_img.copy().shape)
    ...
    def projective_alignment(self):
        ...
        # print("mslee21/lfp_global_resampler/GridFitter/R, K, t", R, K, t)
        del gfxy
        ...
        # adjustment = np.array([1, 2]) if self.cfg.calibs[self.cfg.pat_type] == 'rec' else np.array([2, 1])
        # target_pitch += np.mod(target_pitch + adjustment, 2)
        ...
        # min_grid_0 = min(grid[:, 0])
        # min_grid_1 = min(grid[:, 1])
        # shifts[0] = min(min_grid_0 - cropsets[0], 0)
        # shifts[1] = min(min_grid_1 - cropsets[1], 0)
        # grid[:, :2] -= shifts

        grxy = GridFitter(grid, flip_xy=True)
        grxy.main()
        ...
        # mslee21/oshape=(5100, 5100) set

        oshape = [self._limg_pitch[0]*dims[0],self._limg_pitch[1]*dims[1]]
        # print("mslee21/lfp_global_resampler.py/oshape set=", oshape)
        
        # transform light-field image
        # ### 2023.11.30. original
        # tform = transform.ProjectiveTransform(matrix=tra_mat)
        # self._lfp_prj = transform.warp(self._lfp_img,
        #                                tform.inverse, output_shape=oshape)


        ##### 2023.11.30. changes
        import cv2
        self._lfp_prj = cv2.warpPerspective(self._lfp_img,
                                            tra_mat,
                                            dsize=(oshape[1],oshape[0]))
