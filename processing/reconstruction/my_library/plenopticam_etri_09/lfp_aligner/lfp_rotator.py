...
from plenopticam_etri_09.lfp_calibrator.centroid_drawer import CentroidDrawer
...
class LfpRotator(object):
    ...
    def main(self):
        ...
        print("mslee21/lfp_rotator.py/_estimate_rad : ", self._estimate_rad())
        print("mslee21/lfp_rotator.py/self._rad : ", self._rad)
    ...
    def _estimate_rad(self, regression=True):
        ...
        print("mslee21/lfp_rotator.py/_estimate_radself._rad/ ", self._rad)
    ...
    def _rotate_img(self):
        ...
        #     self._lfp_img = scipy.ndimage.rotate(self._lfp_img, angle=deg_angle, reshape=False, order=0)
        # # print("self._lfp_img.type",self._lfp_img.dtype)

        import cv2
        if deg_angle != 0:
            # 이미지의 중심점을 계산
            image_center = tuple(np.array(self._lfp_img.shape[1::-1]) / 2)

            # 회전을 위한 변환 행렬 생성
            rot_mat = cv2.getRotationMatrix2D(image_center, deg_angle, 1.0)

            # 변환 행렬을 사용하여 이미지 회전
            self._lfp_img = cv2.warpAffine(self._lfp_img, rot_mat, self._lfp_img.shape[1::-1], flags=cv2.INTER_NEAREST)


        ######### 23.11.29. 속도 문제로, pil 모듈 사용
        # from PIL import Image
        # if self._lfp_img.dtype != np.uint8:
        #     # 값의 범위를 0-255로 조정하는 예시
        #     self._lfp_img = (self._lfp_img * 255).astype(np.uint8)
        #
        # print("type(self._lfp_img)",type(self._lfp_img))
        # self._lfp_img = Image.fromarray(self._lfp_img)
        # if deg_angle != 0:
        #     # PIL의 rotate 함수를 사용하여 이미지 회전 (시계 반대 방향)
        #     # expand=True는 이미지 크기를 조정하여 회전된 전체 이미지를 포함하도록 합니다.
        #     # 이 예제에서는 expand를 False로 설정하여 scipy.ndimage의 reshape=False와 유사한 효과를 냅니다.
        #     self._lfp_img = self._lfp_img.rotate(-deg_angle, expand=False)
        #     lfp_img_array = np.array(self._lfp_img)
        #
        #     # 데이터 타입을 'float64'로 변경
        #     self._lfp_img = lfp_img_array.astype('float32')
