...
from plenopticam_etri_09.lfp_aligner.lfp_resampler import LfpResampler
from plenopticam_etri_09.lfp_aligner.lfp_rotator import LfpRotator
from plenopticam_etri_09.lfp_aligner.cfa_outliers import CfaOutliers
from plenopticam_etri_09.lfp_aligner.cfa_processor import CfaProcessor
from plenopticam_etri_09.lfp_aligner.lfp_devignetter import LfpDevignetter
import time
...
class LfpAligner(object):
    ...
    def main(self):
        ...
        if self.cfg.lfpimg:
            ...
            del obj
            print("CfaOutliers time", time.time()-stime)
        ...
        if self.cfg.params[self.cfg.opt_vign] and self._wht_img is not None:
            ...
            del obj
            print("LfpDevignetter time", time.time()-stime)
        ...
        if self.cfg.lfpimg and len(self._lfp_img.shape) == 2:
            ...
            del cfa_obj
            print("CfaProcessor time", time.time()-stime)

        if self.cfg.params[self.cfg.opt_rota] and self._lfp_img is not None:
        # if self._lfp_img is not None:
            # de-rotate centroids
            if self.cfg.params[self.cfg.opt_rota] == True:
                print("rotate")
                obj = LfpRotator(self._lfp_img, self.cfg.calibs[self.cfg.mic_list],
                                 rad=None, cfg=self.cfg, sta=self.sta)
            else:
                print("roatate value")
                obj = LfpRotator(self._lfp_img, self.cfg.calibs[self.cfg.mic_list],
                                 rad=self.cfg.params[self.cfg.val_rota], cfg=self.cfg, sta=self.sta)
            obj.main()
            self._lfp_img, self.cfg.calibs[self.cfg.mic_list] = obj.lfp_img, obj.centroids
            del obj
            print("LfpRotator time", time.time()-stime)
        ...
        del obj
        print("LfpResampler time", time.time()-stime)
