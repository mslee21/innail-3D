...
class LfpCropper(LfpMicroLenses):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self._lfp_img_align is not None:
            ...
            print("lfp_cropper/_size_pitch",self._size_pitch)
            print("lfp_cropper/_limg_pitch[0]", self._limg_pitch[0])
            print("lfp_cropper/_limg_pitch[1]", self._limg_pitch[1])
            # self.new_lfp_img = np.zeros([int(self._limg_pitch[0]*self._LENS_Y_MAX),
            #                              int(self._limg_pitch[1]*self._LENS_X_MAX),
            #                              p],
            #                             dtype=self._lfp_img_align.dtype)
...
# 23.10.21. (RPC, rec)    
    ...
    def lfp_img_align(self):
        # print("mslee21/lfp_cropper.py/self.new_lfp_img.shape,  : ",self.new_lfp_img.shape)
